...
public class TryFlowMetricsModel {
	...
	public TryFlowMetricsModel(String filePath, String project, int invokedMethods, int tryQuantity, int trySizeLOC,
			int trySizeSLOC) {
		super();
		FilePath = filePath;
		Project = project;
	...
	public static void GenerateCSVFromList(List<TryFlowMetricsModel> catchFlowMetricsList, String csvName, String column) {
		try {
			FileWriter writer = new FileWriter(csvName);
			writer.write(column + "\n");
			
			for(TryFlowMetricsModel model : catchFlowMetricsList) {
